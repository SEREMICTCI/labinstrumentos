---
title: "Análisis de redes"
---

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  message = FALSE,
  warning = FALSE,
  fig.width = 12,
  fig.height = 8,
  out.width = "100%",
  dpi = 300,
  # dev = "png",
  comment = "#>"
)
```

```{r}
# Cargamos paquetes sugeridos
library(tm)
library(igraph)
library(data.table)

# Y el paquete con los datos
library(labinstrumentos)

vars <- c("clean_problema", "clean_causa", "clean_consecuencia")

lab_instrumentos[, (vars) := lapply(.SD, gsub, pattern = "macrozona-austral", replacement = ""), .SDcols = vars]
```

# Problemas

```{r}
red_problemas <- crear_redes_de_palabras(
  x = lab_instrumentos$clean_problema
)
```

## Gráfico de redes circular

```{r}
grafico_red_circular(red = red_problemas)
```

## Score de Hubs

```{r}
grafico_red_hubs(red = red_problemas)
```

## Detección de comunidades

### Algorítmo Spinglass

```{r}
red_comunidades_spinglass(red = red_problemas)
```

### Algoritmo de optimización 'Greedy'

```{r}
red_comunidades_greedy(red = red_problemas)
```

# Causas

```{r}
red_causas <- crear_redes_de_palabras(
  x = lab_instrumentos$clean_causa
)
```

## Gráfico de redes circular

```{r}
grafico_red_circular(red = red_causas)
```

## Score de Hubs

```{r}
grafico_red_hubs(red = red_causas)
```

## Detección de comunidades

### Algorítmo Spinglass

```{r}
# red_comunidades_spinglass(red = red_causas) # Arroja un error, explorar por qué pasa esto
```

### Algoritmo de optimización 'Greedy'

```{r}
red_comunidades_greedy(red = red_causas)
```

# Consecuencias

```{r}
red_consecuencias <- crear_redes_de_palabras(
  x = lab_instrumentos$clean_consecuencia
)
```

## Gráfico de redes circular

```{r}
grafico_red_circular(red = red_consecuencias)
```

## Score de Hubs

```{r}
grafico_red_hubs(red = red_consecuencias)
```

## Detección de comunidades

### Algorítmo Spinglass

```{r}
red_comunidades_spinglass(red = red_consecuencias)
```

### Algoritmo de optimización 'Greedy'

```{r}
red_comunidades_greedy(red = red_consecuencias)
```


